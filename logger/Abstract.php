...
namespace vip9008\googleapisclient\logger;
...
abstract class Abstract
{
  ...
  {
    $this->setLevel(
        $client->getClassConfig('Abstract', 'level')
    );

    $format = $client->getClassConfig('Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('Abstract', 'date_format');
